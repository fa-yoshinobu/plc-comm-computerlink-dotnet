...
$projectPath = Join-Path $repositoryRoot "src\Toyopuc\PlcComm.Toyopuc.csproj"
...
try {
    ...
    try {
        $files = @($archive.Entries |
            ...
            Sort-Object -Unique)
        $nuspecEntry = $archive.GetEntry("PlcComm.Toyopuc.nuspec")
    ...
    $required = @(
        "LICENSE",
        "README.md",
        "PlcComm.Toyopuc.nuspec",
        "lib/net8.0/PlcComm.Toyopuc.dll",
        "lib/net8.0/PlcComm.Toyopuc.xml",
        "lib/net9.0/PlcComm.Toyopuc.dll",
        "lib/net9.0/PlcComm.Toyopuc.xml",
        "lib/net10.0/PlcComm.Toyopuc.dll",
        "lib/net10.0/PlcComm.Toyopuc.xml"
    )
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <ItemGroup><PackageReference Include="PlcComm.Toyopuc" Version="$packageVersion" /></ItemGroup>
</Project>
"@)
    [System.IO.File]::WriteAllText($consumerProgram, @"
using System;
using PlcComm.Toyopuc;
Console.WriteLine(typeof(ToyopucClient).FullName);
"@)
