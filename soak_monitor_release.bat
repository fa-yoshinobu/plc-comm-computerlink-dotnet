...
set "OUTPUT_DIR=%ROOT_DIR%artifacts\publish\Toyopuc.SoakMonitor"
set "SKIP_BUILD=0"
...
echo.
echo SoakMonitor publish completed.
...
:usage
echo Usage: soak_monitor_release.bat [--skip-build] [--output PATH]
echo.
echo   --skip-build   Skip dotnet build.
echo   --output PATH  Override publish output. Default is artifacts\publish\Toyopuc.SoakMonitor.
popd >nul
exit /b 0
...
:fail
echo.
echo SoakMonitor publish failed.
popd >nul
exit /b 1
