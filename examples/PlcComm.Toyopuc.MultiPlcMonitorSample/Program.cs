...
if (options.DryRun)
{
    ...
    Console.WriteLine($"cycles: {(options.Cycles is null ? "forever" : options.Cycles.Value.ToString(System.Globalization.CultureInfo.InvariantCulture))}");
    return 0;
}
    ...
    bool DryRun)
{
    ...
    {
        Console.WriteLine("Read the same tag set from multiple TOYOPUC PLCs concurrently.");
        Console.WriteLine("Usage:");
        Console.WriteLine("  dotnet run --project examples/PlcComm.Toyopuc.MultiPlcMonitorSample -- --plc NAME=HOST,PROFILE[,PORT[,TRANSPORT]] [--plc ...] [--tag NAME=ADDRESS]");
    }
}
