...
if (options.DryRun)
{
    ...
    Console.WriteLine($"cycles: {(plan.Cycles is null ? "forever" : plan.Cycles.Value.ToString(System.Globalization.CultureInfo.InvariantCulture))}");
    ...
    bool DryRun)
{
    ...
    {
        Console.WriteLine("Run read-only TOYOPUC polling from a JSON config file.");
        Console.WriteLine("Usage:");
        Console.WriteLine("  dotnet run --project examples/PlcComm.Toyopuc.ConfigPollingSample -- --config examples/PlcComm.Toyopuc.ConfigPollingSample/config_polling.example.json --dry-run");
    ...
    private static List<TagSpec> ParseTags(JsonElement plc, string plcName)
    {
        ...
        foreach (var item in tagsElement.EnumerateArray())
        {
            tags.Add(item.ValueKind == JsonValueKind.String
                ...
                : new TagSpec(GetRequiredString(RequireObject(item, "tag"), "name", "tag.name"), GetRequiredString(item, "address", "tag.address")));
        }
        if (tags.Count == 0)
