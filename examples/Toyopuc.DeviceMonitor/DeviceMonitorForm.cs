...
namespace Toyopuc.DeviceMonitor;
...
internal sealed class DeviceMonitorForm : Form
{
    ...
    public DeviceMonitorForm()
    {
        ...
        _protocolComboBox.Items.AddRange(["tcp", "udp"]);
    ...
    private void WireEvents()
    {
        ...
        _protocolComboBox.SelectedIndexChanged += (_, _) => RefreshEndpointLabel();
    ...
    private ConnectionSettings ReadConnectionSettings()
    {
        ...
        return new ConnectionSettings(
            host,
            port,
            _protocolComboBox.Text.Trim().ToLowerInvariant(),
            localPort,
            timeout,
            retries,
    ...
    private bool ApplyConnectionSettingsDraft(ConnectionSettingsDraft draft)
    {
        ...
        _protocolComboBox.SelectedItem = draft.Protocol;
        if (_protocolComboBox.SelectedIndex < 0)
        {
            _protocolComboBox.Text = draft.Protocol;
        }
    ...
    private void RefreshEndpointLabel(ConnectionSettings settings)
    {
        ...
        _endpointLabel.Text = $"Endpoint: {settings.Protocol}://{settings.Host}:{settings.Port} [{settings.Profile}]{relay}";
        _connectionSummaryLabel.Text =
            $"{settings.Protocol}://{settings.Host}:{settings.Port}  [{settings.Profile}]  interval={_intervalTextBox.Text}s{relay}";
    }
    ...
    {
        var relay = settings.Hops is null ? string.Empty : $" via {settings.Hops}";
        return $"{settings.Protocol}://{settings.Host}:{settings.Port} [{settings.Profile}]{relay}";
    }
